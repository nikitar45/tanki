...
class Shoot:
        ...
        def destroy(self, matrix):
                if self.orient == 1:
                        ...
                        if up == 1:
                                booms.append(
                                    Boom((self.x * 2 + 10, self.y * 2 + 20)))
                                return True
                        elif up == 2:
                                matrix[self.y // 10 + 1][self.x // 10] = 0
                                play.wall = 0
                                booms.append(
                                    Boom((self.x * 2 + 10, self.y * 2 + 10)))
                                return True
                        else:
                                return False
                elif self.orient == 2:
                        ...
                        if left == 1:
                                booms.append(
                                    Boom((self.x * 2 + 20, self.y * 2 + 10)))
                                return True
                        elif left == 2:
                                matrix[self.y // 10][self.x // 10 + 1] = 0
                                play.wall = 0
                                booms.append(
                                    Boom((self.x * 2 + 14, self.y * 2 + 10)))
                                return True
                        else:
                                return False
                elif self.orient == 4:
...
class Player:
        ...
        def walls(self, matrix):
                if (self.ggy % 10) == 0 and (self.ggx % 10) == 0:
                        ...
                        if (Y_axisd == 1 or Y_axisd == 2 or Y_axisd == 4
                            or Y_axisd == 6) and self.orient == 2:
                                self.wall = 2
                        elif (Y_axisu == 1 or Y_axisu == 2 or Y_axisu == 4
                              or Y_axisu == 6) and self.orient == 1:
                                self.wall = 1
                        elif (X_axisr == 1 or X_axisr == 2 or X_axisr == 4
                              or X_axisr == 6) and self.orient == 4:
                                self.wall = 4
                        elif (X_axisl == 1 or X_axisl == 2 or X_axisl == 4
                              or X_axisl == 6) and self.orient == 3:
                                self.wall = 3
        ...
        def touch(self, i):
                if self.orient == 1:
                        if (self.ggx == enemyes[i].x
                            and self.ggy - 10 == enemyes[i].y):
                                self.wall = 1
                                return True
                        return False
                elif self.orient == 2:
                        ...
                        if (self.ggx - 10 == enemyes[i].x
                            and self.ggy == enemyes[i].y):
                                self.wall = 3
                                return True
                        return False
                elif self.orient == 4:
...
class Enemy:
        ...
        def enshoot(self):
                if self.k == 0:
                        if self.orient == 1:
                                shoots.append(
                                    Shoot(self.x, self.y - 4, self.orient, 0))
                        elif self.orient == 2:
                                shoots.append(
                                    Shoot(self.x, self.y + 4, self.orient, 0))
                        elif self.orient == 3:
                                shoots.append(
                                    Shoot(self.x - 4, self.y, self.orient, 0))
                        elif self.orient == 4:
                                shoots.append(
                                    Shoot(self.x + 4, self.y, self.orient, 0))
                        self.k = 50
    ...
    while not go:
        ...
        for index, option in enumerate(options):
            if selected_option == index:
                text_color = (255, 0, 0)  # Красный цвет для выбранного пункта
            else:
                text_color = (255, 255, 255)  # Белый цвет для остальных пунктов

            text_surface = font3.render(option, 1, text_color)
            text_rect = text_surface.get_rect(center=(screen.get_width() // 2, 300 + index * 40))
...
while not done:
        ...
        while m1m < len(shoots):
                ...
                while m2m < len(shoots):
                        ...
                        if shoots[m2m].sight == shoots[m1m].sight:
                                pass
                        elif (shoots[m2m].x == shoots[m1m].x and shoots[m2m].y
                              == shoots[m1m].y) or (dx < 3 and dy < 3):
                                booms.append(
                                    Boom((shoots[m1m].x * 2 + 10,
                                          shoots[m1m].y * 2 + 10)))
                                shoots.pop(m2m)
                                shoots.pop(m1m)
        ...
        text2 = font2.render(u'Осталось врагов: ' + str(p2count), 1,
                             (255, 255, 10))
        textpos2 = text2.get_rect()
        textpos2 = (510, 60)

        text3 = font3.render(u'Вы победили!', 1, (255, 10, 10))
        textpos3 = text3.get_rect(centerx=(screen.get_width() - 140) // 2,
        ...
        textpos5 = text5.get_rect(centerx=(screen.get_width() - 140) // 2,
                                  centery=screen.get_height() // 2)

        text9 = font3.render(u'Вы прошли игру!', 1, (255, 10, 10))
        textpos9 = text5.get_rect(centerx=(screen.get_width()) // 2,
                                  centery=screen.get_height() // 2)

        text6 = font2.render(u'Уровень игрока: ' + str(play.power - 1), 1,
                             (255, 255, 10))
        textpos6 = text2.get_rect()
        textpos6 = (510, 80)

        text7 = font2.render(u'Здоровье игрока: ' + str(play.hp), 1,
                             (255, 255, 10))
        textpos7 = text2.get_rect()
        textpos7 = (510, 100)

        text8 = font2.render(u'Жизни игрока: ' + str(plives), 1,
                             (255, 255, 10))
